...
First, we aim to estimate the expected counts, conditional on selected covariates. In addition to the first two principal component scores derived from external data (ACS and CARTO Spatial Features), we can also add endogenous variables. These include:
